...
public class GsonSerializationProxy implements MongoSerialization {
    private static final Gson _gsonExt = new GsonBuilder()
            .registerTypeAdapter(Dataset.class, new DatasetGsonAdapter())
    		/*egisterTypeHierarchyAdapter(Dataset.class, new DatasetAdapter())*/
    ...
    private static final class DatasetGsonAdapter implements JsonSerializer<Dataset>, JsonDeserializer<Dataset> {
        @Override
        public JsonElement serialize(
                ...
                final JsonSerializationContext context)
        {
            ...
            obj.addProperty("name", dataset.getName());
            obj.addProperty("datasetID", dataset.getId());
            dataset.getDefinition().getKeys().stream()
            .forEach(name -> {
            	dataset.getDefinition().get(name).ifPresent(attr -> {
            ...
            s.getKeys().stream()
                .forEach(name -> {
                    s.get(name).ifPresent(value -> {
                        obj.add(name,context.serialize(value, Value.class));
                    });
                });
            	obj.add("sample", attributes);
            }
            obj.add("samples", attributes);
            return obj;
        }
                ...
                JsonDeserializationContext context)
        {
        	...
        	final JsonObject obj = new JsonObject();
        	 ...
        	 dataset.setName(obj.getAsJsonPrimitive("name").getAsString());
        	 dataset.setId(obj.getAsJsonPrimitive("datasetID").getAsString());
    ...
    private static final class VisualizationGsonAdapter implements JsonDeserializer<Visualization>, JsonSerializer<Visualization> {
        private static class ElementCreator implements Visualization.Visitor {
            ...
            public void visit(Visualization.Histogram hist) {
                ...
                obj.add("attributes", attributes);
                obj.add("data", data);
                obj.addProperty("name", hist.getId());
            ...
            public void visit(Visualization.Scatter scatter) {
                JsonObject obj = new JsonObject();
                obj.addProperty("name", scatter.getId());
            ...
            public void visit(Visualization.Series series) {
                JsonObject obj = new JsonObject();
                obj.addProperty("name", series.getId());
